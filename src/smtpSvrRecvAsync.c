...
void *H_SERVER_WORK_TH(void *args) {
    ...
    while (!g_sys_close) {
        ...
        */
        ...
        session = popSessionByThreadNum(thread_num);

        if (session == NULL) {
            // LOG (LOG_MIN, "no session");
            msleep(25);
            continue;
        }
...
int smtpStartWorkThreads(int n_work_threads) {
    ...
    for (int i = 0; i < n_work_threads; i++) {
        if ((nErr = pthread_create(&clientTh, &clientThAttr, H_SERVER_WORK_TH, (void*)i)) < 0) {
